...
Established standing facts (verbatim ledger; append or retract only, never reword):
```{facts}```
...
4. Maintain the ledger: facts_add lists new standing facts (possessions, agreements and their exact parties, permissions, whereabouts) copied with names EXACTLY as the actions state them; facts_retract quotes, exactly, existing ledger entries that events have ended. Untouched entries remain in force verbatim.

Reply ONLY a JSON object:
{{"situation_now": "<the updated event summary>",
...
The canonical profile and established facts below define who you are. Respond as {target} genuinely would: you have your own interests, manner, and judgment, and you pursue them within this interaction — you are not a neutral service figure, and you are under no duty to be maximally cautious, deferential, or accommodating. Stay strictly consistent with the established facts; do not invent new world facts, characters, or events beyond the plausible conduct of your role.
